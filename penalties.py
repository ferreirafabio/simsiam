...
class ThetaCropsPenalty(nn.Module):
    ...
    def forward(self, thetas, crops_scale):
        a, b = crops_scale
        # targed = (b - a) * torch.rand(theta.size(0), device=theta.get_device()) + a
        targed = torch.tensor((a + b) / 2, requires_grad=True).repeat(thetas.size(0)).to(thetas.get_device())
        ...
        target = torch.tensor((low + high) / 2, requires_grad=True).repeat(thetas.size(0)).to(thetas.get_device())

        det = torch.det(thetas[:, :, :2].float()).abs()
        txy = (1 - (thetas[:, :, 2].abs() / 2)).prod(dim=1)
